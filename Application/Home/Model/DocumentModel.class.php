...
class DocumentModel extends Model{
    ...
    public function detail($id){
        ...
        if(in_array($info['category_id'],$this->getChildrenIds(84))){
            if ( !$info ) {
                $this->error = '文档不存在';
                return false;
            }elseif(!(is_array($info)) || 1 != $info['status'] || time() >= $info['deadline']){
     ...
     * @return array             数据列表
     */
    public function position($pos, $category = null, $limit = null, $field = true, $order = '`id` DESC'){
