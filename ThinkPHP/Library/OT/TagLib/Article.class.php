...
class Article extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'position' => array('attr' => 'pos,cate,child,limit,filed,name', 'close' => 1), //获取推荐位列表
    ...
    public function _list($tag, $content){
        ...
        if($child == 'true') {
            ...
            $parse .= '$__LIST__ = D(\'Document\')->page(!empty($_GET["p"])?$_GET["p"]:1,' . $row . ')->lists(';
            $parse .= $cate.',\'`level` DESC,`id` DESC\', 1,';
        }
        $parse .= $field . ');';
        $parse .= ' ?>';
    ...
    public function _page($tag){
        ...
        if($child == 'true') {
            $parse .= '$category=D(\'Category\')->getChildrenId(' . $cate . ');';
            $parse .= '$__PAGE__ = new \Think\Page(get_list_count($category), ' . $listrow . ');';
        } else {
